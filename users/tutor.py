        ...
        try:
            ...
            if choice == "1":
                ...
                if not available_classes:
                    print("You have no classes assigned.")
                print("\n")
                if yes_or_no(
                ...
                try:
                    choice = int(
                        input(
                            ...
                            " To cancel this action type any other number\n"
                        )
                    )
                    ...
                    if choice == 1:
                        while True:
                            ...
                            if available_classes:
                                choice = int(
                                    input(
                                        "What class would you like to change information of?\n"
                                        " (write 0 if you want to quit)\n"
                                    )
                                )
                                if choice > available_classes[-1]["index"]:
                            ...
                            for available_class in available_classes:
                                if available_class.get("index") == choice:
                                    print(
                                        "Information of this class before the "
                                        "changes:\n",
                                        class_info[
                                            available_class["position"]
                                        ],
                                    )
                                    while True:
                                        key = input(
                                            "What type of info do you want to"
                                            " change?\n"
                                        )
                                        if (
                                                class_info[
                ...
                while True:
                    ...
                    if new_class["name"] == "None":
                        print("You can`t name your class that.")
                        if yes_or_no():
                            break
                        else:
                            continue
                    find = None
                    for class_iterator in class_info:
                        if class_iterator["name"] == new_class["name"]:
                            print(
                                "Class with this name already exists."
                                " You can`t create classes with the same name"
                            )
                            find = yes_or_no()
                    if find:
                        break
                    ...
                    print(
                        "What`re the week days of this class(E.g. mon,tue,fri)"
                    )
                    new_class["dates"] = (
                    ...
                    if class_info[-1]:  # Check if
                        # it`s the first class to be created in the file
                    ...
                    for (
                        ...
                        if user_iterator["email"] == user["email"]:
                            if user_iterator["classes"] == "None":
                                user_iterator["classes"] = new_class["name"]
                            elif isinstance(user_iterator["classes"], str):
                                user_iterator["classes"] += "," + (
                                    str(new_class["name"])
                                )
                            else:
                                user_iterator["classes"].append(
                                    str(new_class["name"])
                                )
                            user = user_iterator
                            break
                ...
                for available_class in available_classes:
                    if available_class.get("index") == choice:
                        ...
                        for (
                            ...
                            # from the database
                            if (
                                    user_iterator["role"] == "Student"
                                    and available_class["name"]
                                    in user_iterator["subjects"]
                            ):
                                user_iterator["subjects"].remove(
                                    available_class["name"]
                                )
                            if (
                                    user_iterator["role"] == "Tutor"
                                    and available_class["name"]
                                    in user_iterator["classes"]
                            ):
